...
export default function SkillCard({ skill }: SkillCardProps) {
  ...
  return (
    <Link href={`/skills/${skill.id}`} className="block group">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs transition-all duration-200 hover:shadow-md hover:border-fuchsia-500 hover:-translate-y-0.5 flex flex-col justify-between h-full">
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-fuchsia-800 bg-fuchsia-100/80 px-2 py-0.5 rounded border border-fuchsia-200">
              <CheckCircle2 className="size-3 text-fuchsia-600" />
              Verified
            </span>
          </div>

          {/* Service Title */}
          <h3 className="text-base font-bold text-slate-900 group-hover:text-fuchsia-700 transition-colors line-clamp-2 leading-snug">
            {skill.title}
          </h3>
        ...
        <div className="mt-5 border-t border-slate-100 pt-3 flex items-center justify-between">
          <div className="tabular-nums">
            ...
            {skill.price_mon && (
              <p className="text-xs font-mono font-bold text-fuchsia-600">{skill.price_mon} MON</p>
            )}
          </div>

          <button className="rounded-lg bg-fuchsia-600 px-3.5 py-1.5 text-xs font-bold text-white group-hover:bg-fuchsia-700 transition-colors shadow-xs flex items-center gap-1">
