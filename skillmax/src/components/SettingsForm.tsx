    ...
    <form onSubmit={handleSave} className="space-y-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
      <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
        <img
          src={form.avatar_url}
          alt="Avatar Preview"
          className="h-16 w-16 rounded-full object-cover border-2 border-fuchsia-500 shadow-xs"
        />
      ...
      {[
        ...
        <div key={f.key}>
          ...
          <input
            ...
            className="w-full rounded-lg border border-slate-300 px-3.5 py-2 text-xs focus:border-fuchsia-600 focus:outline-none text-slate-900"
          />
        </div>
      ))}
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full rounded-lg border border-slate-300 px-3.5 py-2 text-xs focus:border-fuchsia-600 focus:outline-none resize-none text-slate-900"
        />
      </div>

      {error && <p className="text-xs text-red-600 bg-red-50 p-2.5 rounded-lg">{error}</p>}
      {saved && (
        <p className="text-xs text-fuchsia-600 flex items-center gap-1">
      ...
      <button
        type="submit"
        disabled={loading}
        className="w-full rounded-lg bg-fuchsia-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-fuchsia-700 disabled:opacity-50 transition-colors shadow-xs"
      >
