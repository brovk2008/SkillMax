    ...
    <div className="mx-auto max-w-6xl px-4 py-8 space-y-6 bg-white">
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-6 md:p-8 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <span className="inline-flex items-center gap-1 text-xs font-semibold uppercase tracking-wider text-fuchsia-800 bg-fuchsia-100/80 px-2.5 py-0.5 rounded-full">
              <ClipboardList className="h-3.5 w-3.5 text-fuchsia-700" />
          ...
          <div className="flex gap-2 shrink-0">
            <Link
              href="/tasks/new"
              className="rounded-lg bg-fuchsia-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-fuchsia-700 shadow-xs flex items-center gap-1.5"
            >
        ...
        <form action="/tasks" method="GET" className="flex gap-2 pt-2">
          ...
          <button
            type="submit"
            className="rounded-xl bg-fuchsia-600 px-5 py-2 text-xs font-bold text-white hover:bg-fuchsia-700 transition-colors shadow-xs"
          >
            Search
          </button>
        </form>
      ...
      {tasks && tasks.length > 0 ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {tasks.map((task: any) => {
            ...
            const avatar = client?.avatar_url || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80'

            return (
              <div
                key={task.id}
                className="rounded-xl border border-slate-200 bg-white p-5 shadow-xs hover:border-fuchsia-500 hover:shadow-md transition-all flex flex-col justify-between"
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="inline-flex items-center gap-1 text-[11px] font-bold text-fuchsia-700 bg-fuchsia-50 px-2 py-0.5 rounded border border-fuchsia-200">
                      Open Request
                    </span>
                  </div>
                ...
                <div className="mt-5 border-t border-slate-100 pt-3 flex items-center justify-between">
                  <div>
                    ...
                    {task.budget_mon && (
                      <p className="text-xs font-mono font-semibold text-fuchsia-600">{task.budget_mon} MON</p>
                    )}
                  </div>

                  <Link
                    href={`/messages`}
                    className="rounded-lg bg-fuchsia-600 px-3.5 py-1.5 text-xs font-bold text-white hover:bg-fuchsia-700 transition-colors shadow-xs flex items-center gap-1"
                  >
