...
### `SkillMaxEscrow.sol`
Located at `contracts/src/SkillMaxEscrow.sol`:
```solidity
...
### `SkillMaxBadge.sol`
Located at `contracts/src/SkillMaxBadge.sol`:
```solidity
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.24;
...
Located at `supabase/migrations/001_initial.sql` & SQL queries:
...
### Webhook Verification (`src/app/api/razorpay/webhook/route.ts`)
```typescript
...
Located at `src/lib/achievements.ts`:
